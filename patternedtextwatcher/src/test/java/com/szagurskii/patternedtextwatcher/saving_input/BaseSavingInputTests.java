...
public abstract class BaseSavingInputTests extends BaseTests {
    ...
    public void shouldSaveOnSingleAdding() {
        for (String pattern : PATTERNS) {
            ...
            addText(e);
            assertText(E + s + p + r + e, E + s + p + r + e, patternedTextWatcher.getFullString(), pattern);
            addText(s);
            assertText(E + s + p + r + e + s, E + s + p + r + e + s, patternedTextWatcher.getFullString(), pattern);
            addText(s);
            assertText(E + s + p + r + e + s + s, E + s + p + r + e + s + s, patternedTextWatcher.getFullString(), pattern);
            addText(o);
            assertText(E + s + p + r + e + s + s + o, E + s + p + r + e + s + s + o, patternedTextWatcher.getFullString(), pattern);
    ...
    protected void assertText(String typed, String expected, String actual, String pattern) {
        // Here we strip the possible unnecessary expected string because we know that we should assert string that is equal to the pattern in length.
        super.assertText(typed, pattern.length() < expected.length() ? expected.substring(0, pattern.length()) : expected, actual, pattern);
    }
}
